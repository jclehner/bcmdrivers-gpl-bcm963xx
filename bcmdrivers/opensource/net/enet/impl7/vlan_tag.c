...
{
    if (demux_on_sw(self->p.parent_sw, self->p.port_id, self))
        return -1;
    
    if (mux_on_sw(self, self->p.parent_sw, self->p.port_id, self->p.parent_sw->s.parent_port))
        return -1;
...
sw_ops_t port_vlan_sw =
{
    ...
    .port_demux = port_vlan_sw_demux,
    .port_mux = port_vlan_sw_mux,
