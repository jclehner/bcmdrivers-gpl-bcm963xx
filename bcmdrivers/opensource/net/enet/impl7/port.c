...
int port_is_enet_link_handling(enetx_port_t *p)
{
    ...
    return ((phy && (phy->phy_drv->phy_type == PHY_TYPE_SF2_SERDES || 
...
int port_init(enetx_port_t *port)
{
    ...
    if (port->p.phy)
    {
        ...
        {
            if (is_cascade_phy(port->p.phy))
            {
                phy_dev_t *phy = cascade_phy_get_last(port->p.phy);
                phy_dev_link_change_register(phy, dslbase_phy_link_change_cb, phy);    // only register callback for last of cascade phy
            }
            else
                phy_dev_link_change_register(port->p.phy, dslbase_phy_link_change_cb, port->p.phy);
        }
#else
...
init_ports:
    for (i = 0; i < sw->s.port_count; i++)
    {
        ...
        if (sw->s.ports[i]->port_class & (PORT_CLASS_PORT_DETECT | PORT_CLASS_PORT_UNDEF))
            continue;
...
int port_generic_sw_demux(enetx_port_t *sw, enetx_rx_info_t *rx_info, FkBuff_t *fkb, enetx_port_t **out_port)
{
    *out_port = sw->s.demux_map[rx_info->src_port];
    ...
    {
        enet_err("can't demux rx_port %d!!\n", rx_info->src_port);
        return -1;
    }
