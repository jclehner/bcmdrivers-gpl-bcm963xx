...
static inline uint8 *enet_sysport_alloc_data_buf(void)
{
    ...
    /* Move the pointer to data packet (leaving headroom + FKB space)*/
    pData = PFKBUFF_TO_PDATA(pBuf, BCM_PKT_HEADROOM);
    return pData;
}

static inline void enet_sysport_free_data_buf(uint8 *pData)
{
    void *p_bpm_buf = (void *) PDATA_TO_PFKBUFF(pData,BCM_PKT_HEADROOM);
    gbpm_free_buf(p_bpm_buf);
}
...
static uint32 enet_sysport_rx_pkts(uint32 budget)
{
    ...
    for(; --budget > 0; )
    {
        ...
        blogAction = blog_finit_locked( pFkb, dev, TYPE_ETH, 0, BLOG_ENETPHY, &fc_args);
...
static void __init reset_sysport(void)
{
    /* TBD - figure out why flush causes problems */
    /* Reset Receive Path */
    //SYSPORT_TPC->SYSTEMPORT_TOPCTRL_RX_FLUSH_CNTL = (1<<0);
    /* Reset Transmit Path */
    //SYSPORT_TPC->SYSTEMPORT_TOPCTRL_TX_FLUSH_CNTL = (1<<0);
}
