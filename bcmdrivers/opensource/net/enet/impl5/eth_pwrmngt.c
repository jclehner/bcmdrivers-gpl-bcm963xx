...
void ethsw_eee_port_enable(int log_port, int enable, int linkstate)
{
   ...
   if (unit == 1 && !IsExtPhyId(phy_id)) {
      ...
      if (enable) {
         /* Check if 100Base-T Bit[1] or 1000Base-T EEE Bit[2] was negotiated 7.803e */
         /* This step is not essential since the PHY does this already */
         ethsw_phyport_c45_rreg(log_port, 7, 0x803e, &data16);
         if (data16 & 0x6) {
            v16 |= (1<<phys_port);
         }
      } else {
...
#if defined(CONFIG_BCM963148)
      val16 = (eth_eee_enabled)?2:0; // Support only 100 BT EEE
#else
