...
static int bcme_create_rdpa_wan_gbe_port(void)
{
    ...
    rdpa_port_index_set(rdpa_port_attrs, rdpa_wan_type_to_if(rdpa_wan_gbe));
...
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148) || defined(CONFIG_BCM94908)
    /* Runner based device with external switch - only supports WAN port on Runner */
    if (is_ext_sw && LOGICAL_PORT_TO_UNIT_NUMBER(logical_port) == 0)
    {
        /* Note : Comparing wan_port_id with logical port w/ assumption that wan_port_id is equal to logical port value */
...
int bcmeapi_create_vport(struct net_device *dev)
{
    ...
    if (init_cfg.gbe_wan_emac != rdpa_emac_none && dev->base_addr == init_cfg.gbe_wan_emac)
    {
        ...
        pDevCtrl->rdpa_port = rdpa_wan_type_to_if(rdpa_wan_gbe); 
        return 0;
    }
    ...
    rdpa_cpu_rxq_rx_isr_cb_t rx_isr)
{
    ...
    rc = rdpa_cpu_rx_create_ring(&enet_ring[queue_id - NETDEV_CPU_RX_QUEUE_ID],queue_size,&ring_base);
    if (rc < 0)
       goto unlock_exit;
...
unlock_exit:
    ...
    if (!rc) 
    {
       printk(KERN_NOTICE "Created Ethernet Rx queue %d with size %d, Descriptor base=%pK\n", 
...
{
    int vport = LOGICAL_PORT_TO_VPORT( log_port );
    struct net_device* dev;
    BcmEnet_devctrl *pDevCtrl;

    if ( vport < 0 || vport > MAX_NUM_OF_VPORTS )
    {
        printk("\n%s() - Error : Invalid vport %d for log_port=%d\n\n",__FUNCTION__, vport, log_port);
        return rdpa_if_none;
    }
    dev = vnet_dev[vport];
    if (!dev)
    {
        printk("\n%s() - Error : NULL dev : Invalid vport %d for log_port=%d\n\n",__FUNCTION__, vport, log_port);
        return rdpa_if_none;
    }
    pDevCtrl = (BcmEnet_devctrl *)netdev_priv(dev);

    if (pDevCtrl->rdpa_port < rdpa_if_first || pDevCtrl->rdpa_port >= rdpa_if__number_of)
    {
        printk("\n%s() - Error : Invalid rdpa_port %d for log_port=%d\n\n",__FUNCTION__, pDevCtrl->rdpa_port, log_port);
        return rdpa_if_none;
    }

    return pDevCtrl->rdpa_port;
}
...
void bcmeapi_EthGetStats(int log_port, uint32 *rxDropped, uint32 *txDropped)
{
    ...
    if (rc)
    {
        printk("%s: can not get port id %d \n", __FUNCTION__, _rdpa_if);
        goto unlock_exit;
    }
