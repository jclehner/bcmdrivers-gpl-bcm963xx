             ...
             int wl_radio_idx)
{
    ...
    if (maxQIdx < WFD_NUM_QUEUE_SUPPORTED)
    {
        ...
        kthread_bind(wfd_objects[tmp_idx].wfd_rx_thread, tmp_idx % 2);
        ...
        printk("\033[1m\033[34m %s: Dev %s wfd_idx %lu wl_radio_idx %d Type %s configured WFD thread %s "
            ...
            "minQId/maxQId (%d/%d), status (%d) qmask 0x%x cpu %lu \033[0m\n",
            ...
            wfd_get_qid(minQIdx), wfd_get_qid(maxQIdx), rc, wfd_objects[tmp_idx].wfd_queue_mask, tmp_idx % 2);
    }
    else
    {
