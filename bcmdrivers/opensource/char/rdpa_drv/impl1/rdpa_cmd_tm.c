    ...
    rdpa_stat_1way_t *pstats)
{
    ...
    if (dev_type == RDPA_IOCTL_DEV_PORT) {
        ...
        if (dir == rdpa_dir_us) {
            if (dev_id == rdpa_wan_type_to_if(rdpa_wan_dsl)) {
                /* For xtmWan, the queue's channel id is q_id off by rdpa_if */
                q_index.channel = rdpa_wan_type_to_if(rdpa_wan_dsl) + q_id; 
            }
            else {
#if defined(CONFIG_BCM_PON)
                ...
                q_index.channel = rdpa_if_wan0; /* FIXME: MULTI-WAN DSL */ 
#endif
            }
        }
        else {
            /* ethLan channel id is its emac id.
                      ...
                      * etc....
                      */
            q_index.channel = dev_id - rdpa_if_wan1; /* FIXME: MULTI-WAN DSL */ 
        }
    }
#if defined(CONFIG_BCM_PON)
    ...
    uint32_t *pq_occupancy)
{
    ...
    /* get queue occupancy */
    if ((rc = rdpa_egress_tm_queue_occupancy_get(sched, &q_index, pq_occupancy)) != BDMF_ERR_OK)
    {
    ...
    case RDPA_IOCTL_TM_CMD_TM_CONFIG: {
        if (tm.dev_type == RDPA_IOCTL_DEV_NONE) {
            ...
            if (tm.arbiter_mode == rdpa_tm_sched_disabled) {
                ...
                CMD_TM_LOG_DEBUG("SET tm(shaping_rate=[%llu], burst=[%llu]\n",
                                 (uint64_t)rl_cfg.af_rate, (uint64_t)rl_cfg.burst_size);
