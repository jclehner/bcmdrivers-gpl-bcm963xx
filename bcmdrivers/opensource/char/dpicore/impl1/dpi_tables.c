...
void dpi_reset_stats(void)
{
	...
	hash_for_each(appinst_table.list, i, entry, node) {
		memset(&entry->us, 0, sizeof(entry->us));
		memset(&entry->ds, 0, sizeof(entry->ds));
	}
...
#define init_table(name, struct_name) \
	({ \
		...
		hash_init(name.list); \
		name.cache = KMEM_CACHE(struct_name, GFP_ATOMIC); \
		if (!name.cache) { \
...
{
	if (init_table(app_table, dpi_app))
		goto err;
	if (init_table(dev_table, dpi_dev))
		goto err_free_app_table;
	if (init_table(appinst_table, dpi_appinst))
		goto err_free_dev_table;
	if (init_table(url_table, dpi_url))
