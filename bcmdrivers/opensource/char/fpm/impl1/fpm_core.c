...
int fpm_get_pool_stats(int pool, struct fpm_pool_stats *stats)
{
	...
	stats->overflow_count		= FPM_GET_UNDERFLOW(pool);
...
static irqreturn_t fpm_isr(int irq, void *dev)
{
	...
	for (pool = 0; pool < 2; pool++) {
		status = FPM_GET_IRQ_MASK(pool) & FPM_GET_IRQ_STATUS(pool);
		if (!status)
			continue;
...
/*	  			FPM_FREE_TOKEN_INDEX_OUT_OF_RANGE_MASK | */
			...
			FPM_SET_IRQ_MASK(0, mask);
#if FPM_POOL_NUM > 1
			if (fdev->pool_pbase[1])
				FPM_SET_IRQ_MASK(1, mask);
#endif
