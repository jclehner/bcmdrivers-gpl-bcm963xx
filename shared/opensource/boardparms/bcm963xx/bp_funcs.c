...
static bp_pinmux_defs_t g_pinmux_defs_0[] = {
  ...
  { bp_usUart1Sdin,     -1, 35, 35 | BP_PINMUX_VAL_3},
  { bp_usUart1Sdout,    -1, 36, 36 | BP_PINMUX_VAL_3},
  { bp_usUart1Sdin,     -1, 54, 54 | BP_PINMUX_VAL_3},
  { bp_usUart1Sdout,    -1, 55, 55 | BP_PINMUX_VAL_3},
...
static bp_pinmux_defs_t g_pinmux_defs_0[] = {
  ...
  { bp_usGpioUart2Cts,  -1, 20, 20 | BP_PINMUX_VAL_2},
  { bp_usGpioUart2Rts,  -1, 21, 21 | BP_PINMUX_VAL_2},
  { bp_usGpioUart2Sdout,-1, 22, 22 | BP_PINMUX_VAL_2},
  { bp_usGpioUart2Sdin, -1, 23, 23 | BP_PINMUX_VAL_2},
  ...
  { bp_usSfpSigDetect,  0, 10, 10 | BP_PINMUX_VAL_6 }, //xPON
  { bp_usSfpSigDetect,  0, 37, 37 | BP_PINMUX_VAL_3 },
...
int BpGetLedsAdvancedInfo(LEDS_ADVANCED_INFO *pLedsInfo)
{
    ...
    for (i = 0, pE = pLedsInfo; i < BP_MAX_SWITCH_PORTS; i++)
    {
        for ( j = 0; j < 4; j++ )
        {
            pE->ledInfo[i].SpeedLed[j] = BP_NOT_DEFINED;
            pE->ledInfo[i].ActivityLed[j] = BP_NOT_DEFINED;
        }
    }
    ...
    if (g_pCurrentBp)
    {
        ...
        for( i = 0, pE = pLedsInfo; i < BP_MAX_ENET_MACS; i++ )
        {
            ...
            for( j = 0; j < BP_MAX_SWITCH_PORTS; j++ )
            {
                ...
                bp_current_phyid = bp_aulPhyId[j];
                pelem = BpGetElem(bp_current_phyid, &pPhyId, bp_next);
                pE->ledInfo[j].SpeedLed[0]    = 0;
                pE->ledInfo[j].SpeedLed[1]    = 0;
                pE->ledInfo[j].SpeedLed[2]    = 0;
                pE->ledInfo[j].SpeedLed[3]    = 0;
                pE->ledInfo[j].ActivityLed[0] = 0;
                pE->ledInfo[j].ActivityLed[1] = 0;
                pE->ledInfo[j].ActivityLed[2] = 0;
                pE->ledInfo[j].ActivityLed[3] = 0;
                if (bp_current_phyid == pelem->id)
                {
                    ++pPhyId;
                    while (pPhyId) {
