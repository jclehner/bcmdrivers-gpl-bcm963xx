...
int bcm_init_pinmux(void)
{
    ...
    if ( (PERF->RevID & REV_ID_MASK) != 0xa1 )
    {
        for (i = n-1 ; 0 <= i ; i--)
        {
            if ( (((Function[i] & BP_GPIO_NUM_MASK) == 5) && ((Muxinfo[i] & BP_PINMUX_VAL_MASK) == BP_PINMUX_VAL_1)) ||
                 (((Function[i] & BP_GPIO_NUM_MASK) == 9) && ((Muxinfo[i] & BP_PINMUX_VAL_MASK) == BP_PINMUX_VAL_3)) ||
                 (((Function[i] & BP_GPIO_NUM_MASK) == 54) && ((Muxinfo[i] & BP_PINMUX_VAL_MASK) == BP_PINMUX_VAL_2)) ||
                 (((Function[i] & BP_GPIO_NUM_MASK) == 52) && ((Muxinfo[i] & BP_PINMUX_VAL_MASK) == BP_PINMUX_VAL_2)) )
            {
                Muxinfo[i] &= (BP_PINMUX_PIN_MASK | BP_PINMUX_VAL_MASK);
            }
        }
    }
#endif
