...
{
     char *serdes_tx_dis_usage = "Usage: 10G_SFP_TX_DIS state <state 0|1>";
     ...
     if (kstrtos32(argv[1], 10, &gpio_state))
        goto error;

     if (gpio_state > 1)
	 goto error;
	
     write_gpio_52_state(gpio_state); 
...
static int serdes_proc_cmd_txfir(int argc, char *argv[])
{
    uint32_t port_id;
    uint16_t pre, main, post1, post2;
    char *serdes_proc_cmd_txfir_usage = "Usage: txfir <port (0-6)> [<pre (0-10)> <main (1-60)> <post1> <post2 (0-5)>] \n\
    \n\
    ...
    if (argc == 2)
    {
        lport_serdes_get_txfir(port_id, &pre, &main, &post1, &post2);
        pr_info("pre   %d\n", pre);
        pr_info("main  %d\n", main);
        pr_info("post1 %d\n", post1);
        pr_info("post2 %d\n", post2);
    ...
    return lport_serdes_set_txfir(port_id, pre, main, post1, post2);

wrong_params:
    pr_info("%s\n", serdes_proc_cmd_txfir_usage);
    return -1;
}
...
static struct proc_cmd_ops serdes_command_entries[] = {
    ...
    { .name = "txfir",      .do_command = serdes_proc_cmd_txfir},
};
