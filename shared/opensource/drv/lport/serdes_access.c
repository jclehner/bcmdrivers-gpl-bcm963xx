...
static unsigned short tx_dis_gpio;
...
{
    bcm_gpio_set_data(tx_dis_gpio, state);
	return 0;
}
...
static E_MERLIN_VCO merlin_vco[2] = {};

int merlin_core_init(E_MERLIN_ID core_id, E_MERLIN_VCO vco)
{
    lport_srds_dual_serdes_0_cntrl serdes0_cntrl;
    lport_srds_dual_serdes_1_cntrl serdes1_cntrl;

    if (merlin_vco[core_id] == vco)
        return LPORT_ERR_OK;
...
static void serdes_work_cb(struct work_struct *work)
{
    ...
    uint32_t port = serdes_work->info->entity_id;

    bcm_gpio_set_data(tx_dis_gpio, 0);
...
static void remote_fault_handler(const lport_intr_info_s *info, void *priv)
{
    ...
    bcm_gpio_set_data(tx_dis_gpio, 1);
...
int lport_serdes_init(lport_init_s *init_params)
{
    ...
    for (i = 0 ; i < LPORT_NUM_OF_PORTS; i++ )
    {
        ...
        if (LPORT_IS_SERDES_PORT(prt_mux_sel))
        {
            ...
            {
                if (prt_mux_sel == PORT_HSGMII)
                    vco = MERLIN_VCO_9375_MHZ;
                else
                    vco = MERLIN_VCO_103125_MHZ;

                rc = merlin_core_init(SERDES_LANE_TO_CORE(port_2_lane_idx[i]), vco);
                if (rc)
                {
                    pr_err("Init of merlin core for port %d failed\n", i);
                    return LPORT_ERR_STATE;
                }
            ...
            merlin_callbacks.merlin_lane_init(port_2_lane_idx[i], prt_mux_sel);
        }
    }

#ifndef _CFE_
    if (BpGet10GTxDisGpio(&tx_dis_gpio) != BP_SUCCESS)
        return LPORT_ERR_OK;
    ...
    for (i = 0; i < 2; i++)  
    {
        ...
        {
            lport_intr_enable(LPORT_REMOTE_FAULT, i, 0);
            lport_intr_unregister(LPORT_REMOTE_FAULT, i);
        }
    }
#endif
...
{
    return merlin_callbacks.merlin_speed_set(port_2_lane_idx[port], rate);
}
...
int lport_serdes_set_txfir(uint32_t port, uint16_t pre, uint16_t main, uint16_t post1, uint16_t post2)
{
    merlin_control_t control = {};
    control.txfir.pre = pre;
    control.txfir.main = main;
    control.txfir.post1 = post1;
    control.txfir.post2 = post2;
    return merlin_callbacks.merlin_ioctl(port_2_lane_idx[port], MERLIN_CMD_FIR_SET, &control);
}

int lport_serdes_get_txfir(uint32_t port, uint16_t *pre, uint16_t *main, uint16_t *post1, uint16_t *post2)
{
    int rc;
    merlin_control_t control = {};
    
    rc = merlin_callbacks.merlin_ioctl(port_2_lane_idx[port], MERLIN_CMD_FIR_GET, &control);
    if (rc)
    {
    ...
    *pre   = control.txfir.pre;
    *main  = control.txfir.main;
    *post1 = control.txfir.post1;
    *post2 = control.txfir.post2;
exit:    
    return rc;
}
