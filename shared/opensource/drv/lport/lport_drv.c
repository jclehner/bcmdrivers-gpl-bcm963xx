...
int validate_lport_configuration(lport_init_s *init_params)
{
    ...
    for (iter = 0; iter < LPORT_NUM_OF_PORTS; iter++)
    {
        ...
        if (iter != 0 && iter != 4 && prt_mux_sel[iter] == PORT_XFI)
        {
...
static LPORT_PORT_RATE lport_speed_get(uint8_t portid)
{
...
int msbus_init(lport_init_s *init_params)
{
    ...
    for (i = 0; i < LPORT_NUM_OF_XLMACS; i++) 
    {
        rc = rc ? rc : ag_drv_lport_mab_cntrl_get(i, &cntrl_xlmac);
        rc = rc ? rc : ag_drv_lport_mab_tx_wrr_ctrl_get(i, &wrr);
        cntrl_xlmac.gmii_rx_rst = 0xf;
        cntrl_xlmac.gmii_tx_rst = 0xf;
        cntrl_xlmac.xgmii_rx_rst = 0x1;
        cntrl_xlmac.xgmii_tx_rst = 0x1;
        wrr.p4_weight = 0;
        wrr.p5_weight = 0;
        wrr.p6_weight = 0;
        wrr.p7_weight = 0;
        ...
        for (j = 0; j < LPORT_NUM_OF_PORTS_PER_XLMAC; j++)
        {
            ...
            if (init_params->prt_mux_sel[idx] != PORT_UNAVAIL)
            {
                ...
                {
                    cntrl_xlmac.xgmii_rx_rst = 0x0;
                    cntrl_xlmac.xgmii_tx_rst = 0x0;
                    wrr.p4_weight = 2;
                }
                else
                {
                    cntrl_xlmac.gmii_rx_rst &= ~(1 << j);
                    cntrl_xlmac.gmii_tx_rst &= ~(1 << j);
        ...
        rc = rc ? rc : ag_drv_lport_mab_cntrl_set(i , &cntrl_xlmac);
    }
...
int lport_get_port_configuration(uint32_t portid, lport_port_cfg_s *port_conf)
{
    ...
    port_conf->tx_throttling = xlmac_tx_ctrl.throt_denom ? 1 : 0;
...
int lport_set_port_configuration(uint32_t portid, lport_port_cfg_s *port_conf)
{
    ...
    lport_xlmac_tx_ctrl xlmac_tx_ctrl;
    lport_xlmac_rx_ctrl xlmac_rx_ctrl;
    LPORT_PORT_MUX_SELECT  prt_mux_sel = local_lport_cfg.prt_mux_sel[portid];
    LPORT_PORT_MUX_SELECT  new_mux_sel = prt_mux_sel;
    lport_init_s lport_cfg = local_lport_cfg;
    ...
    if ((prt_mux_sel == PORT_SGMII) || (prt_mux_sel == PORT_HSGMII) || (prt_mux_sel == PORT_XFI))
    {
        ...
        if (prt_mux_sel != new_mux_sel)
        {
            ...
            rc = lport_reinit_driver(&lport_cfg, 0);
        }
    ...
    xlmac_tx_ctrl.throt_denom = port_conf->tx_throttling ? 0x3f : 0x00;
    xlmac_tx_ctrl.throt_num = port_conf->tx_throttling ? 0x3f : 0x00;
...
int lport_port_eee_set(uint32_t portid, uint8_t enable)
{
    ...
    if (enable)
    {
        ...
        case 4: /* 5000/10000Mbps */
            {
                eee_wake_timer = 0x0f; /* 15 uS */
                eee_delay_entry_timer = 0x1e; /* 30 uS */
                break;
            }
        default:
            return -1;
