...
int mii_caps_get(phy_dev_t *phy_dev, uint32_t *pcaps)
{
    ...
    if (bmcr & BMCR_ANENABLE)
    {
        ...
        if (ctrl1000 & ADVERTISE_REPEATER)
    ...
    else
    {
        ...
        if (speed == 0)
            caps |= bmcr & BMCR_FULLDPLX ? PHY_CAP_10_FULL : PHY_CAP_10_HALF;

        if (speed == 1)
            caps |= bmcr & BMCR_FULLDPLX ? PHY_CAP_100_FULL : PHY_CAP_100_HALF;

        if (speed == 2)
            caps |= bmcr & BMCR_FULLDPLX ? PHY_CAP_1000_FULL : PHY_CAP_1000_HALF;
    }
