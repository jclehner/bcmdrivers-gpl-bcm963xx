...
int xg_serdes_inited;
static int xgae_wan_top_init(void);
DEFINE_MUTEX(serdes_mutex);

#define SerdesPrint(x, y...) if(!xg_serdes_inited) printk(x, ##y)
static int poll_pll_lock(int pll_inst, int poll_cnt)
{
    int pll_lock;
    int pll_lock_cnt;
    int pll_error;
    ...
    SerdesPrint("pa_comment: Polling for PLL%0d Lock on Serdes\n", pll_inst);
    ...
    while ((pll_lock == 0) && (pll_lock_cnt < poll_cnt)) {
        ...
        SerdesPrint("pa_comment: serdes_pll_lock pll_inst=%0d poll_cnt=%0d pll_lock=%0d\n", pll_inst, pll_lock_cnt, pll_lock);
        pll_lock_cnt = pll_lock_cnt + 1;
    }
    if (pll_lock == 0) {
        printk("pa_comment: *************** ERROR serdes_pll_lock_error pll_inst=%0d poll_cnt=%0d pll_lock=%0d\n", pll_inst, pll_lock_cnt, pll_lock);
        pll_error = 1;
    } else {
        SerdesPrint("pa_comment: serdes_pll_locked pll_inst=%0d poll_cnt=%0d pll_lock=%0d\n", pll_inst, pll_lock_cnt, pll_lock);
    }
    clr_mask_read_data();
...
static int poll_pll_lock_wan(int pll_inst, int poll_cnt)
{
...
#define LOCK_POLL_ENABLE
    ...
    itr = 0;
    while(1) {
        SerdesPrint("pa_comment: poll_pll_lock_wan: SGB_SERDES_INST: Polling PLL lock .. %0d/10\n", itr);
        REG_READ_32(&WAN_TOP->WAN_TOP_SERDES_STATUS, rd_data);
        if (((pll_inst == 0) & ((rd_data & 0x00000001) != 0)) ||
                ((pll_inst == 1) & ((rd_data & 0x00000400) != 0)) ) {
            SerdesPrint("pa_comment: poll_pll_lock_wan: SGB_SERDES_INST: PLL lock achieved!!\n");
            break;
        } else {
            if (itr++ == 40) {
    ...
    SerdesPrint("pa_comment: poll_pll_lock_wan LOCK_FIXED_WAIT\n");

    sim_wait_time_nsec = 20000;
    printk("pa_comment: SIM_WAIT %d ns\n", sim_wait_time_nsec);
    serdes_wait(sim_wait_time_nsec);
...
static int serdes_enable (int serdes_speed_mode, int serdes_rate_mode)
{
    ...
    SerdesPrint("pa_comment: serdes_enable serdes_speed_mode:%d serdes_rate_modee:%d\n",
            serdes_speed_mode, serdes_rate_mode);

    sim_wait_time_nsec = 10000;
    SerdesPrint("pa_comment: SIM_WAIT %d ns\n", sim_wait_time_nsec);
    serdes_wait(sim_wait_time_nsec);
    ...
    if ((rd_data & 0xffffffff) == wr_data) {
        SerdesPrint("pa_comment: 0, MISC, 0 written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    } else {
    ...
    if ((rd_data & 0xffffffff) == wr_data) {
        SerdesPrint("pa_comment: 0, MISC, 1 written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    } else {
    ...
    if ((rd_data & 0xffffffff) == wr_data) {
        SerdesPrint("pa_comment: 0, MISC, 2 written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    } else {
    ...
    SerdesPrint("pa_comment: CONFIG_BCM963158 0, MISC, 3 written: wr_data=0x%08x\n", wr_data);
    ...
    if ((rd_data & 0xffffffff) == wr_data) {
        SerdesPrint("pa_comment: 0, MISC, 3 written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    } else {
    ...
    if ((rd_data & 0xffffffff) == wr_data) {
        SerdesPrint("pa_comment: 0, WAN_SERDES, PLL_CTL written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    } else {
    ...
    if ((rd_data & 0xffffffff) == wr_data) {
        SerdesPrint("pa_comment: 0, WAN_SERDES, PRAM_CTL written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    } else {
    ...
    if (xg_serdes_inited == 0) {
        SerdesPrint("pa_comment: Test SerDes register access\n");
...
int ethsw_xgae_speed_set(phy_dev_t *phy_dev, phy_speed_t speed, phy_duplex_t duplex)
{
    ...
    xgae_wan_top_init();
    serdes_enable (serdes_speed_mode, serdes_rate_mode);
    ...
    //
    SerdesPrint("pa_comment: Configure AMS_COM_PLL_INTCTRL\n");
    ...
    //
    //
    SerdesPrint("pa_comment: Configure CKRST_CTRL_PLL_SELECT_CONTROL\n");
    if (tx_pll_id == 0) {
    ...
    SerdesPrint("pa_comment: Configure AMS_COM_PLL_CONTROL_1/AMS_COM_PLL_CONTROL_4\n");
    ...
    // Enable PLL's
    SerdesPrint("pa_comment: Configure CORE_PLL_COM_TOP_USER_CONTROL\n");
    ...
    //
    SerdesPrint("pa_comment: Configure DSC_A_cdr_control_0\n");
    ...
    SerdesPrint("pa_comment: Configure DSC_A_cdr_control_2\n");
    ...
    SerdesPrint("pa_comment: Configure DSC_B_dsc_sm_ctrl_7\n");
    ...
    //
#if 0
    SerdesPrint("pa_comment: start skip pa trans\n");
    SerdesPrint("pa_comment: Configure DSC_B_dsc_sm_ctrl_4 SIM_SPEEDUP start:\n");
    wr_xgae_serdes_reg(SERDES_0, (DEVID_1 | LANE_BRDCST | DSC_B_dsc_sm_ctrl_4), 0x0422, ~(0x7fff)); // PMD RX LOCK speedup
    SerdesPrint("pa_comment: Configure DSC_B_dsc_sm_ctrl_4 SIM_SPEEDUP done.\n");
    SerdesPrint("pa_comment: end skip pa trans\n");
#endif
    ...
    // 9.953G  XGPON   9.953G      32      (311.04M) 110       64      (155.52M)    110
    // 1.244G  GPON    2.488G      16      (155.52M) 100       16      (155.52M)    011
    SerdesPrint("pa_comment: Configure AMS_TX_TX_CONTROL_1\n");
    ...
    //
    SerdesPrint("pa_comment: Polling for Tx PLL lock ..\n");
    poll_pll_lock(tx_pll_sel, 10000);

    SerdesPrint("pa_comment: Polling for Rx PLL lock ..\n");
    poll_pll_lock(rx_pll_sel, 10000);

    SerdesPrint("pa_comment: Polling for Tx PLL lock from WAN ..\n");
    poll_pll_lock_wan(tx_pll_sel, 10000);

    // poll_pll_lock_wan(rx_pll_sel, 10000);

    // # 14. De-assert ln_dp_s_rstb bit (per lane)
    //
    // write ( "ln_dp_s_rstb",                                  1);
    //
    SerdesPrint("pa_comment: Configure DIG_COM_TOP_USER_CONTROL_0\n");
    wr_xgae_serdes_reg(SERDES_0, (DEVID_1 | LANE_BRDCST | DIG_COM_TOP_USER_CONTROL_0), 0x2000, ~(0x2000)); // core_dp_s_rstb = 1
    sim_wait_time_nsec = 1000;
    SerdesPrint("pa_comment: SIM_WAIT %d ns\n", sim_wait_time_nsec);
    serdes_wait(sim_wait_time_nsec);

    SerdesPrint("pa_comment: Configure CKRST_CTRL_LANE_CLK_RESET_N_POWERDOWN_CONTROL\n");
    wr_xgae_serdes_reg(SERDES_0, (DEVID_1 | LANE_BRDCST | CKRST_CTRL_LANE_CLK_RESET_N_POWERDOWN_CONTROL), 0x0002, ~(0x0002));

    SerdesPrint("pa_comment: Configure AMS_TX_TX_CONTROL_0\n");
    ...
    wait_cnt_max = 12;
    wait_cnt = 0;
    SerdesPrint("pa_comment: Fixed wait, waiting %dus", wait_cnt_max);
    while (wait_cnt < wait_cnt_max) {
        SerdesPrint("waiting 1us, wait_cnt:%d", wait_cnt);
        sim_wait_time_nsec = 1000;
        SerdesPrint("pa_comment: SIM_WAIT %d ns\n", sim_wait_time_nsec);
        serdes_wait(sim_wait_time_nsec);
        wait_cnt = wait_cnt + 1;
    ...
    //} else {
    SerdesPrint("pa_comment: Configure DSC_F_ONU10G_looptiming_ctrl_0\n");
    ...
    SerdesPrint("pa_comment: Configure TLB_TX_tx_pi_loop_timing_config\n");
    wr_xgae_serdes_reg(SERDES_0, (DEVID_1 | LANE_BRDCST | TLB_TX_tx_pi_loop_timing_config), 0x0001, ~(0x0001)); // tx_pi_loop_timing_src_sel

    SerdesPrint("pa_comment: Configure TX_PI_LBE_tx_pi_control_0\n");
    ...
    SerdesPrint("pa_comment: Read SERDES MICRO_A_temperature_status\n");
    set_mask_read_data(0x00000000);
    rd_data = rd_xgae_serdes_reg(SERDES_0, (DEVID_1 | LANE_BRDCST | MICRO_A_temperature_status));
    SerdesPrint("pa_comment: Read SERDES MICRO_A_temperature_status rd_data:0x%04x\n", rd_data);
    clr_mask_read_data();

    SerdesPrint("pa_comment: Read SERDES PLL_CAL_COM_CTL_STATUS_0 - Rescal\n");
    set_mask_read_data(0x00000000);
    rd_data = rd_xgae_serdes_reg(SERDES_0, (DEVID_1 | LANE_BRDCST | PLL_CAL_COM_CTL_STATUS_0));
    SerdesPrint("pa_comment: Read SERDES PLL_CAL_COM_CTL_STATUS_0 - Rescal rd_data:0x%04x\n", rd_data);
    clr_mask_read_data();


    SerdesPrint("pa_comment: Done with Serdes Init\n");
    SerdesPrint("pa_comment: END wan_top_ral_sequence wan_top_misc INIT_SERDES\n");




    // Configure AMS PLL Control to Disable PTEST output
    SerdesPrint("pa_comment: SGB_SERDES: Disable PTEST output.\n");
    ...
    set_mask_read_data(0x00000000);
    SerdesPrint("pa_comment: SGB_SERDES_INST: Reading AMS PLL status.\n");
    rd_data = rd_xgae_serdes_reg(SERDES_0, (DEVID_1 | LANE_BRDCST | AMS_COM_PLL_STATUS));
    SerdesPrint("pa_comment: SGB_SERDES_INST: AMS PLL status = 0x%04x\n", rd_data);
    clr_mask_read_data();


    // Read PLL CAL COM status for ATE test
    set_mask_read_data(0x00000000);
    SerdesPrint("pa_comment: SGB_SERDES_INST: Reading PLL CAL COM status.\n");
    rd_data = rd_xgae_serdes_reg(SERDES_0, (DEVID_1 | LANE_BRDCST | PLL_CAL_COM_CTL_STATUS_0));
    SerdesPrint("pa_comment: SGB_SERDES_INST: PLL CAL COM status = 0x%04x\n", rd_data);
    clr_mask_read_data();

    // Read AMS_TX_TX_STATUS status for ATE test
    set_mask_read_data(0x00000000);
    SerdesPrint("pa_comment: SGB_SERDES_INST: Reading AMS_TX_TX_STATUS status.\n");
    rd_data = rd_xgae_serdes_reg(SERDES_0, (DEVID_1 | LANE_BRDCST | AMS_TX_TX_STATUS));
    SerdesPrint("pa_comment: SGB_SERDES_INST: AMS_TX_TX_STATUS status = 0x%04x\n", rd_data);
    clr_mask_read_data();


    // Enable MICRO for ATE test
    SerdesPrint("pa_comment: De-assert MICRO reset.\n");
    ...
    set_mask_read_data(0x00000000);
    SerdesPrint("pa_comment: SGB_SERDES_INST: Reading MICRO_A_temperature_status.\n");
    rd_data = rd_xgae_serdes_reg(SERDES_0, (DEVID_1 | LANE_BRDCST | MICRO_A_temperature_status));
    SerdesPrint("pa_comment: SGB_SERDES_INST: MICRO_A_temperature_status = 0x%04x\n", rd_data);
    clr_mask_read_data();
    ...
    SerdesPrint("pa_comment: SGB_SERDES: Speed up values for pmd_rx_lock only for simulation. Please remove for silicon test.\n");
    SerdesPrint("pa_comment: Configure DSC_B_dsc_sm_ctrl_4 SIM_SPEEDUP start:\n");
    wr_xgae_serdes_reg(SERDES_0, (DEVID_1 | serdes_addr | DSC_B_dsc_sm_ctrl_4), 0x0422, 0x8000);
    SerdesPrint("pa_comment: Configure DSC_B_dsc_sm_ctrl_4 SIM_SPEEDUP done.\n");

    SerdesPrint("pa_comment: SGB_SERDES: Speed up simulation for TX disable.\n");
    SerdesPrint("pa_comment: Configure TX_FED_misc_control1 SIM_SPEEDUP start:\n");
    wr_xgae_serdes_reg(SERDES_0, (DEVID_1 | serdes_addr | TX_FED_misc_control1),  0x0000, 0xff03);
    SerdesPrint("pa_comment: Configure TX_FED_misc_control1 SIM_SPEEDUP done.\n");

    SerdesPrint("pa_comment: end skip pa trans\n");
#endif

    SerdesPrint("pa_comment: SGB_SERDES: Ignore signal detect indication in DSC SM.\n");
    SerdesPrint("pa_comment: Configure DSC_B_dsc_sm_ctrl_0\n");
    ...
    SerdesPrint("pa_comment: SERDES_MICROCODE\n");
    error_cnt = error_cnt + serdes_microcode (serdes_speed_mode, serdes_rate_mode);
#endif // SERDES_MICROCODE

#ifdef SERDES_DIG_LOOPBACK
    SerdesPrint("pa_comment: SGB_SERDES: Enable DIGITAL LOOPBACK.\n");
    SerdesPrint("pa_comment: Configure TLB_RX_DIG_LPBK_CONFIG\n");
    ...
    if (prbs_mode < 7) {
        SerdesPrint("pa_comment: SGB_SERDES: Enable PRBS generator, PRBS %d (mode %d).\n", prbs_type, prbs_mode);
        SerdesPrint("pa_comment: Configure TLB_TX_prbs_gen_config\n");
        wr_xgae_serdes_reg(SERDES_0, (DEVID_1 | serdes_addr | TLB_TX_prbs_gen_config), (0x0001 | (prbs_mode << 1)), 0xfe00);

        SerdesPrint("pa_comment: SGB_SERDES: Enable PRBS checker clock.\n");
        SerdesPrint("pa_comment: Configure TLB_RX_prbs_chk_config\n");
        wr_xgae_serdes_reg(SERDES_0, (DEVID_1 | serdes_addr | TLB_RX_prbs_chk_config), 0x0800, 0xf7ff);

        SerdesPrint("pa_comment: SGB_SERDES: Configure PRBS checker, PRBS %d (mode %d).\n", prbs_type, prbs_mode);
        SerdesPrint("pa_comment: Configure TLB_RX_prbs_chk_config\n");
    ...
    SerdesPrint("pa_comment: serdes_init LOCK_POLL_ENABLE\n");
    serdes_wait(10000);
    ...
    set_mask_read_data(0x00000000);
    while (1) {
        SerdesPrint("pa_comment: serdes_init: SGB_SERDES_INST: Polling for Rx DSC Lock .. %0d/10\n", itr);
        SerdesPrint("pa_comment: Read DSC_B_dsc_sm_status_dsc_lock\n");
        rd_data = rd_xgae_serdes_reg(SERDES_0, (DEVID_1 | serdes_addr | DSC_B_dsc_sm_status_dsc_lock));
        if ((rd_data & 0x00000001) != 0) {
            SerdesPrint("pa_comment: serdes_init: SGB_SERDES_INST: Rx DSC Lock achieved!!\n");
            break;
        } else {
            if (itr++ == 40) {
    ...
    SerdesPrint("pa_comment: serdes_init LOCK_FIXED_WAIT\n");

    sim_wait_time_nsec = 20000;
    SerdesPrint("pa_comment: SIM_WAIT %d ns\n", sim_wait_time_nsec);
    serdes_wait(sim_wait_time_nsec);

    SerdesPrint("pa_comment: SGB_SERDES_INST: Checking for Rx DSC Lock .. \n");
    SerdesPrint("pa_comment: Read DSC_B_dsc_sm_status_dsc_lock\n");
    set_mask_read_data(0x00000001);
    rd_data = rd_xgae_serdes_reg(SERDES_0, (DEVID_1 | serdes_addr | DSC_B_dsc_sm_status_dsc_lock));
    clr_mask_read_data();
    if ((rd_data & 0x00000001) != 0) {
        SerdesPrint("pa_comment: SGB_SERDES_INST: Rx DSC Lock achieved!!\n");
    } else {
    ...
    SerdesPrint("pa_comment: SGB_SERDES: Forcing burst_valid 1G .. \n");
    wr_xgae_serdes_reg(SERDES_0, (DEVID_1 | serdes_addr | DSC_G_BURST_SM_CTL_0), 0x1c14, 0x0000);
    } else {
    SerdesPrint("pa_comment: SGB_SERDES: Forcing burst_valid 10G .. \n");
    ...
    set_mask_read_data(0x00000000);
    while (1) {
        SerdesPrint("pa_comment: SGB_SERDES_INST: Polling for Rx PMD lock .. %0d/10\n", itr);
        SerdesPrint("pa_comment: Read TLB_RX_pmd_rx_lock_status\n");
        rd_data = rd_xgae_serdes_reg(SERDES_0, (DEVID_1 | serdes_addr | TLB_RX_pmd_rx_lock_status));
        if ((rd_data & 0x00000001) != 0) {
            SerdesPrint("pa_comment: SGB_SERDES_INST: Rx PMD Lock achieved!!\n");
            SerdesPrint("\n");
            break;
        } else {
            if (itr++ == 40) {
    ...
    set_mask_read_data(0x00000001);
    SerdesPrint("pa_comment: SGB_SERDES_INST: Checking for Rx PMD lock.\n");
    SerdesPrint("pa_comment: Read TLB_RX_pmd_rx_lock_status\n");
    set_mask_read_data(0x00000001);
    rd_data = rd_xgae_serdes_reg(SERDES_0, (DEVID_1 | serdes_addr | TLB_RX_pmd_rx_lock_status));
    clr_mask_read_data();
    ...
    if (serdes_speed_mode == eSPEED_10_10) {
        SerdesPrint("pa_comment: 0, AE_GEARBOX_CONTROL_0, _0, first time written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    ...
    if ((rd_data & 0xffffffff) == wr_data) {
        SerdesPrint("pa_comment: 0, AE_GEARBOX_CONTROL_0, _0 written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    } else {
    ...
    if ((rd_data & 0xffffffff) == wr_data) {
        SerdesPrint("pa_comment: 0, TOP_RESET, T written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    } else {
    ...
    if (serdes_speed_mode == eSPEED_10_10) {
        ...
        SerdesPrint("pa_comment: BEGIN Programing PCS registers for 10G \n");
        ...
        SerdesPrint("pa_comment: END Programing PCS registers for 10G \n");

    }else if (serdes_speed_mode == eSPEED_100m_100m) {
        rd_data = rd_xgae_serdes_reg(SERDES_1, (DEVID_0 | LANE_BRDCST | ieee0Blk_MIICntl));
        wr_data = rd_data;
        wr_data = wr_data | 0x2000; // manual_speed1 = bit 13--> 1
        wr_data = wr_data & 0xffff; // manual_speed0 = 0
        SerdesPrint("pa_comment: Programing PCS register ieee0Blk_MIICntrl register for 100m mode \n");
        ...
        wr_data = wr_data & 0xffff; // manual_speed0 = 0
        SerdesPrint("pa_comment: Programing PCS register FX100_Control1 register for 100m mode \n");
        ...
        SerdesPrint("pa_comment: Programing PCS register FX100_Control3 register for 100m mode \n");
        ...
        SerdesPrint("pa_comment: END Programing PCS registers for 100M mode \n");
    };

    //  pcs_init(serdes_speed_mode);
    SerdesPrint("pa_comment: Done with SGB_SERDES_INST Serdes Config!!\n");
...
static int xgae_wan_top_init(void)
{
    ...
    SerdesPrint("pa_comment: SGB_SERDES_INST: Serdes PMD reset.\n");
    ...
    if ((rd_data & 0xffffffff) == wr_data) {
        SerdesPrint("pa_comment: 0, PMI, LP_0 written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    } else {
    ...
    if ((rd_data & 0xffffffff) == wr_data) {
        SerdesPrint("pa_comment: 0, PMI, LP_1 written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    } else {
    ...
    if ((rd_data & 0xffffffff) == wr_data) {
        SerdesPrint("pa_comment: 0, PMI, LP_2 written: wr_data=0x%08x rd_data=0x%08x\n", wr_data, rd_data);
    } else {
    ...
    SerdesPrint("pa_comment: 0, PMI, LP_3 read: rd_data=0x%08x\n", rd_data);
