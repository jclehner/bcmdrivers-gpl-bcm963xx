...
/*
 * Phy driver for external 1G/2.5G/5G/10G phys: BCM8486x, BCM8488x
 */
...
firmware_t mako_a0 = { bcm8486x_a0_version, bcm8486x_a0_firmware, sizeof(bcm8486x_a0_firmware) };
#endif
#ifdef _ORCA_A0_
#include "bcm8488x_a0_firmware.h"
firmware_t orca_a0 = { bcm8488x_a0_version, bcm8488x_a0_firmware, sizeof(bcm8488x_a0_firmware) };
#endif
#ifdef _ORCA_B0_
#include "bcm8488x_b0_firmware.h"
firmware_t orca_b0 = { bcm8488x_b0_version, bcm8488x_b0_firmware, sizeof(bcm8488x_b0_firmware) };
#endif
...
static int _phy_idle_stuffing_mode_set(phy_dev_t *phy_dev)
{
    int ret;
    uint16_t enable = 0;
    uint16_t disable = 1;

    /* Enable idle stuffing mode in 5GBASE-T mode only */
    if ((ret = cmd_handler(phy_dev, CMD_SET_XFI_2P5G_5G_MODE, &disable, &enable, NULL, NULL, NULL)))
        goto Exit;
...
static int _phy_pair_swap_set(phy_dev_t *phy_dev)
{
    int ret;
    uint16_t data;
...
static int _phy_init(phy_dev_t *phy_dev)
{
    ...
    if ((ret = _phy_idle_stuffing_mode_set(phy_dev)))
        goto Exit;
    ...
    /* Set pair-swap mode */
    if ((ret = _phy_pair_swap_set(phy_dev)))
        goto Exit;
...
static int _firmware_load(phy_desc_t *phy)
{
    int i, cnt, step, ret;
