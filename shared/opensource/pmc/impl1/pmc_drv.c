...
void set_pmc_boot_param(int boot_option, int boot_param)
{
	...
	printk ("Option address %p:%p\n", bootOptions, pmcBootParams);
	// set boot options
	switch (boot_option) {
...
static void pmc_boot(void)
{
    ...
    PMC->ctrl.addr1WndwMask = ~(MAX_PMC_ROM_SIZE-1);
    PMC->ctrl.addr1WndwBaseIn = 0x1fc00000;
...
{
    // adjust SWREG
    PROCMON->SSBMaster.wr_data = 0x1234;
    PROCMON->SSBMaster.wr_data = 0x4829;
    PROCMON->SSBMaster.control = 0xb427;
    PROCMON->SSBMaster.wr_data = 0x4829;
    PROCMON->SSBMaster.control = 0xb447;
    PROCMON->SSBMaster.wr_data = 0x5172;
    PROCMON->SSBMaster.control = 0xb443;
    PROCMON->SSBMaster.wr_data = 0x5170;
    PROCMON->SSBMaster.control = 0xb423;
    PROCMON->SSBMaster.wr_data = 0xc690;
    PROCMON->SSBMaster.control = 0xb440;
    PROCMON->SSBMaster.wr_data = 0xc692;
    PROCMON->SSBMaster.control = 0xb440;
    PROCMON->SSBMaster.wr_data = 0xc690;
    PROCMON->SSBMaster.control = 0xb440;
    PROCMON->SSBMaster.wr_data = 0xc690;
    PROCMON->SSBMaster.control = 0xb420;
    PROCMON->SSBMaster.wr_data = 0xc692;
    PROCMON->SSBMaster.control = 0xb420;
    PROCMON->SSBMaster.wr_data = 0xc690;
    PROCMON->SSBMaster.control = 0xb420;
...
#define TEST(x)  {\
                     int num;\
                     for(num=1000;(((*(volatile unsigned int*) 0xffb20060) & 0x8000) && (num > 0)) ; num--) ;\
                         if(!num) \
                         {\
...
#if defined (_BCM96846_) && defined(CFG_RAMAPP)
    dump_swregs();
#endif
